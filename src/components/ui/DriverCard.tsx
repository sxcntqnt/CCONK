...
import { formatTime } from '@/lib/names';
//import { Driver, Bus, DriverCardProps } from "@/utils/constants/types";

import { DriverCardProps } from '@/utils/constants/types';
...
const DriverCard = ({ item, selected, setSelected }: DriverCardProps) => {
    ...
    return (
        <div
            onClick={setSelected}
            onKeyDown={(e) => e.key === 'Enter' && setSelected()}
            className={`flex flex-row items-center justify-between py-5 px-4 mb-4 border rounded-xl relative cursor-pointer transition ${
                selected === item.id ? 'bg-blue-100 border-blue-500' : 'border-gray-300 hover:bg-gray-100'
            } ${aeonik.variable}`}
            role="button"
            tabIndex={0}
        ...
        >
            ...
            <Image
                src={item.profileImageUrl || '/images/default-profile.jpg'}
            ...
            <Image
                src={item.busImageUrl || '/images/default-bus.jpg'}
                alt={`${item.licensePlate} bus`}
                width={56}
                height={56}
