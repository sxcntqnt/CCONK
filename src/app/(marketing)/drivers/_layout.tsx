...
const TabIcon: React.FC<{ source: StaticImageData; focused: boolean; alt: string }> = ({ source, focused, alt }) => (
    <div className={`flex justify-center items-center rounded-full ${focused ? 'bg-gray-300' : ''} p-2`}>
                ...
                <body className="bg-gray-100">
                    ...
                    <nav className="fixed bottom-5 left-0 right-0 mx-5 bg-gray-800 rounded-full h-20 flex justify-between items-center px-6 shadow-lg">
                        {tabs.map((tab) => {
                            ...
                            return (
                                <Link key={tab.name} href={`/${tab.name}`} className="flex-1 flex justify-center">
