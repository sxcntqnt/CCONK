  ...
  try {
    ...
    const busCreations = validCapacities
      ...
      .map(async (capacity) => {
        ...
        const randomLetters = String.fromCharCode(65 + Math.floor(Math.random() * 26)) + // A-Z
...
export async function getBuses(page: number = 1, pageSize: number = 10): Promise<{
  ...
  try {
    ...
    const [buses, total] = await Promise.all([
      prisma.bus.findMany({
        ...
        orderBy: { id: 'asc' },
      }).catch(err => { throw new Error(`findMany failed: ${err.message}`); }),
      prisma.bus.count().catch(err => { throw new Error(`count failed: ${err.message}`); }),
    ]);

    if (buses.length === 0 && total === 0) {
      console.log('No buses found, calling ensureBusExists');
      await ensureBusExists().catch(err => { throw new Error(`ensureBusExists failed: ${err.message}`); });
      const [newBuses, newTotal] = await Promise.all([
        prisma.bus.findMany({
          ...
          select: { id: true, licensePlate: true, capacity: true },
          orderBy: { id: 'asc' },
        }).catch(err => { throw new Error(`post-ensure findMany failed: ${err.message}`); }),
        prisma.bus.count().catch(err => { throw new Error(`post-ensure count failed: ${err.message}`); }),
      ]);
...
export async function getSeats(busId: number): Promise<Record<string, SeatData>> {
  ...
  try {
    ...
    if (busWithSeats.seats.length === 0) {
      ...
      const updatedBusWithSeats = await prisma.bus.findUnique({
        where: { id: busId },
        select: {
          seats: {
            select: {
...
function buildSeatMap(
  seats: { id: number; seatNumber: number; price: number; row: number; column: number; category: string; reservations: { id: number }[] }[],
  totalSeats: number
...
async function initializeSeats(busId: number, capacity: number): Promise<void> {
  ...
  const seatsToCreate = config.layout.flatMap((row, rowIndex) =>
    row.flat().filter(Boolean).map((seatNumber, seatIndex) => ({
      busId,
...
export async function cleanupBusData(busId: number = 1): Promise<void> {
  try {
    await prisma.$transaction([
      prisma.reservation.deleteMany({ where: { seat: { busId } } }),
      prisma.seat.deleteMany({ where: { busId } }),
    ]);
  } catch (error) {
