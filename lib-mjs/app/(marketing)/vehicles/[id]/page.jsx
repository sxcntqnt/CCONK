...
export default function VehicleDetailsPage({ params }) {
    ...
    return (<div className="min-h-screen bg-gray-900 text-white">
            <div className="container mx-auto px-4 py-8">
                <Card className="bg-gray-800 border-gray-700 rounded-lg vehicle-details-card" ref={cardRef} aria-label={`Details for ${bus.licensePlate}`}>
                    <CardHeader>
                        <CardTitle className="text-2xl font-bold">{config.title}</CardTitle>
                        <p className="text-sm text-gray-300">Category: {bus.category}</p>
                    </CardHeader>
                    <CardContent>
                        <VehicleImagesCarousel images={bus.images} licensePlate={bus.licensePlate} className="carousel-container"/>
                    </CardContent>
                </Card>
            </div>
        </div>);
