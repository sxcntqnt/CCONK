...
import { TESTIMONIALS, FLAM_USE_CASES } from '@/utils/constants/misc';
    ...
    return (<div className="overflow-x-hidden scrollbar-hide size-full">
            ...
            <MaxWidthWrapper className="py-10">
                ...
                <AnimationContainer delay={0.2}>
                    <BentoGrid>
                        {CARDS.map((card, idx) => (<BentoCard key={idx} name={card.name} className={card.className} background={card.background} Icon={card.Icon} description={card.description} href={card.href} cta={card.cta}/>))}
                    </BentoGrid>
                </AnimationContainer>
            </MaxWidthWrapper>
            ...
            <MaxWidthWrapper className="py-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 w-full py-8 gap-4 md:gap-8 min-w-0" aria-label="Steps to use FLAM for matatu management">
                    {FLAM_PROCESS.map((process, id) => (<AnimationContainer delay={0.2 * id} key={id}>
            ...
            <MaxWidthWrapper className="py-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 place-items-start gap-4 md:gap-8 py-10 min-w-0" aria-label="User testimonials for FLAM">
                    {TESTIMONIALS.length > 0 ? (TESTIMONIALS.map((testimonial, index) => (<AnimationContainer delay={0.2 * index} key={index}>
                                <MagicCard className="md:p-0">
                                    <Card className="flex flex-col w-full border-none h-min transition-shadow hover:shadow-lg animate-wiggle">
                                        ...
                                        <CardFooter className="w-full space-x-1 mt-auto">
                                            {Array.from({ length: testimonial.rating }, (_, i) => (<StarIcon key={i} className="h-4 w-4 fill-yellow-500 text-yellow-500"/>))}
                                        </CardFooter>
                                    </Card>
                                </MagicCard>
                            </AnimationContainer>))) : (<p className="text-center text-[hsl(var(--muted-foreground))]">No testimonials available</p>)}
                </div>
            </MaxWidthWrapper>
            ...
            <MaxWidthWrapper className="py-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 py-10 min-w-0" aria-label="FLAM use cases for public transportation">
                    {FLAM_USE_CASES.length > 0 ? (FLAM_USE_CASES.map((useCase, index) => (<AnimationContainer delay={0.2 * index} key={index}>
                            ...
                            </AnimationContainer>))) : (<p className="text-center text-[hsl(var(--muted-foreground))]">No use cases available</p>)}
                </div>
            </MaxWidthWrapper>
