...
const TripDetailsPage = () => {
    ...
    return (<RideLayout title={`Trip ${id}`}>
            <div className="px-5 pb-10">
                {loading ? (<div className="flex justify-center">
                        <div className="animate-spin h-6 w-6 border-4 border-blue-500 border-t-transparent rounded-full"/>
                    </div>) : error ? (<p className="text-red-500 text-center">{error}</p>) : trip ? (<TripCard trip={trip}/>) : (<p className="text-gray-600 text-center">No trip data available</p>)}
            </div>
        </RideLayout>);
};
