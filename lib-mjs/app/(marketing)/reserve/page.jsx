...
const BusImageDisplay = ({ imageUrl, category, isLoading, }) => {
    ...
    return (<div className="flex justify-center">
            <Image src={imageUrl} alt={category} width={192} height={192} className="object-cover rounded-md" placeholder="blur" blurDataURL="/placeholder.jpg" priority={false} onLoadingComplete={() => console.log(`Loaded image for ${category}`)} onError={() => toast.error(`Failed to load image for ${category}`)}/>
        </div>);
};
/**
...
export default function ReservePage() {
    ...
    const layout = matatuConfigs[busCapacity]?.layout || matatuConfigs['14'].layout;
    ...
    return (<ErrorBoundary>
            <MaxWidthWrapper className="py-6">
                ...
                {selectedBusId && seatCount > 0 && layout.length > 0 ? (<div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
                        <div>
                            <DynamicSeatLayout ref={seatContainerRef} title={selectedBus.category} seats={seatsForLayout} layout={layout} onSeatClick={(id) => handleSeatClick(id.toString())} isLoading={isLoading} className="mt-2"/>
                ...
                <Dialog open={isCheckoutModalOpen} onOpenChange={(isOpen) => {
            if (!isLoading) {
                ...
                if (!isOpen && paymentSuccess) {
                    ...
                    <DialogContent ref={modalRef} className="bg-gray-900 text-white border-none">
                        ...
                        <DialogFooter className="flex justify-end gap-4">
                            <Button variant="outline" onClick={() => setIsCheckoutModalOpen(false)} disabled={isLoading} className="text-white border-gray-700 w-24">
                                {paymentSuccess ? 'Close' : 'Cancel'}
                            </Button>
                            {!paymentSuccess && (<Button onClick={handleConfirmCheckout} disabled={isLoading || total === 0 || !clerkId} className="bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 w-48">
...
const BookingSummaryCard = ({ selectedSeats, seats, total, phoneNumber, setPhoneNumber, handleCheckout, handleReset, handleSeatClick, isLoading, isPhoneValid, error, }) => {
    return (<Card className="bg-gray-900 text-white border-none shadow-lg">
            <CardHeader>
            ...
            <CardContent>
                ...
                <ScrollArea className="h-40 mb-4">
                    {selectedSeats.length > 0 ? (selectedSeats.map((id) => (<div key={id} className="flex justify-between items-center py-2">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <Button variant="ghost" size="sm" onClick={() => handleSeatClick(id)} disabled={isLoading}>
                                        Cancel
                                    </Button>
                                </div>
                ...
                <div className="flex gap-4">
                    <Button onClick={handleCheckout} disabled={isLoading || selectedSeats.length === 0 || !isPhoneValid || total < 20} className="flex-1 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700">
    ...
    return (<Card className="bg-gray-900 text-white border-none shadow-lg">
            ...
            <CardContent>
                <div className="mb-4">
                    ...
                    <Select value={selectedCapacity} onValueChange={handleCapacityChange} disabled={isLoading}>
                        <SelectTrigger id="capacitySelect" className="mt-1 bg-gray-800 border-gray-700 text-white focus:ring-2 focus:ring-blue-500">
                            <SelectValue placeholder="Select capacity"/>
                        </SelectTrigger>
                        <SelectContent className="bg-gray-800 text-white border-gray-700">
                            <SelectItem value="all">All Capacities</SelectItem>
                            {Object.keys(matatuConfigs).map((capacity) => (<SelectItem key={capacity} value={capacity}>
                                    {matatuConfigs[capacity].title}
                                </SelectItem>))}
                        </SelectContent>
                    </Select>
                </div>
                <div className="mb-4">
                    ...
                    <Input id="licensePlateInput" value={licensePlateFilter} onChange={(e) => handleLicensePlateChange(e.target.value)} placeholder="Enter license plate (e.g., KAA 123B)" disabled={isLoading} className="mt-1 bg-gray-800 border-gray-700 text-white placeholder-gray-500"/>
            ...
            handleLicensePlateChange('');
                ...
                <Pagination>
                    <PaginationContent>
                        <PaginationItem>
                            <PaginationPrevious onClick={handlePrevPage} href="#" className={cn(currentPage === 1 || isLoading ? 'pointer-events-none opacity-50' : '')}/>
                        </PaginationItem>
                        <PaginationItem>
                        ...
                        <PaginationItem>
                            <PaginationNext onClick={handleNextPage} href="#" className={cn(currentPage === totalPages || isLoading ? 'pointer-events-none opacity-50' : '')}/>
                        </PaginationItem>
                    </PaginationContent>
                </Pagination>
            </CardContent>
