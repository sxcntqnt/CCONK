...
export default function PassengerDashboardClient({ userData, passenger, buses, error, role }) {
    ...
    if (error || !passenger || hasReservations === null) {
        return (<div className="flex items-center justify-center flex-col h-screen bg-gray-950">
                ...
                {error && (<Button variant="outline" className="mt-4 text-white border-gray-700 hover:bg-gray-600" onClick={() => router.refresh()}>
                        Retry
                    </Button>)}
            </div>);
    }
    return (<main className="flex-1 bg-gray-950">
            <MaxWidthWrapper className="py-8">
                ...
                <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                                ...
                                <ScrollArea className="h-40">
                                    {buses.length > 0 ? (buses.map((bus) => (<div key={bus.id} className="flex justify-between items-center py-3">
                                                ...
                                                <Link href={`/reserve?busId=${bus.id}`} legacyBehavior>
                                                    <Button variant="outline" size="sm" className="text-white border-gray-700 hover:bg-gray-600 transition-colors">
                                                        Reserve
                                                    </Button>
                                                </Link>
                    ...
                    {passenger.reservations.map((reservation) => (<MagicCard key={reservation.tripId} className="p-0 h-full w-full max-w-xl">
                            <Card className="group bg-gray-900 text-white border-none shadow-lg transition-all duration-300 hover:shadow-xl">
                                <CardHeader className="p-6">
                                ...
                                <CardContent className="p-6 pt-0">
                                    <ScrollArea className="h-40">
                                        <RealTimeTripUpdates tripId={reservation.tripId} driverId={reservation.trip.bus.id.toString()}/>
                ...
                <div className="mt-8 flex justify-center gap-4">
                    <Link href="/" legacyBehavior>
                        <Button variant="outline" className="text-white border-gray-700 hover:bg-gray-600 w-32 transition-colors">
                            Back to Home
                        </Button>
                    </Link>
                    <Button onClick={() => signOut({ redirectUrl: '/' })} className="bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 w-32 transition-colors">
                        Sign Out
                    </Button>
                </div>
            </MaxWidthWrapper>
