...
export default function VehicleImagesCarousel({ images, licensePlate, className }) {
    ...
    return (<Card className={cn('bg-gray-800 border-gray-700 overflow-hidden', className)} ref={carouselRef} tabIndex={0} role="region" aria-label={`Image carousel for ${licensePlate}`}>
            <div className="w-full">
                <div className="relative overflow-hidden">
                    <div className="aspect-square sm:aspect-[4/3] relative">
                        {images.map((image, index) => (<motion.div key={`${licensePlate}-${index}`} initial={{ opacity: 0 }} animate={{
                opacity: index === currentIndex ? 1 : 0,
                zIndex: index === currentIndex ? 10 : 0,
            }} transition={{ duration: 0.3 }} className="absolute inset-0">
                                <Image src={image.src} alt={image.alt || `${licensePlate} view ${index + 1}`} fill className="object-cover" loading={index === 0 ? 'eager' : 'lazy'} placeholder={image.blurDataURL ? 'blur' : 'empty'} blurDataURL={image.blurDataURL || undefined} sizes="(max-width: 768px) 100vw, 800px"/>
                            </motion.div>))}
                    </div>
