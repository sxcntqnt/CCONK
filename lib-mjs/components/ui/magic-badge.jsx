...
const MagicBadge = ({ children, variant = 'default', className }) => {
    ...
    return (<div className={cn('relative inline-flex h-8 select-none overflow-hidden rounded-full p-[1.5px] focus:outline-none', className)}>
            <span className={cn('absolute inset-[-1000%] animate-[spin_3s_linear_infinite]', variant === 'default' &&
            ...
            <span className={cn('inline-flex h-full w-full cursor-pointer items-center justify-center rounded-full px-4 py-1 text-sm font-medium backdrop-blur-3xl', variantStyles[variant])}>
                {children}
            </span>
        </div>);
};
