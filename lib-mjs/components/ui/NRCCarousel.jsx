...
const NRCCarousel = ({ frames, breakpoint = 'lg', slideDuration, noAutoPlay, heights, loadingComponent, blurQuality, noBlur, ariaLabel, controlsComponent, willAutoPlayOutsideViewport, }) => {
    ...
    const carouselIsInViewport = useIsVisible(containerRef) || willAutoPlayOutsideViewport;
    ...
    >
            <div className={clsx({ 'motion-safe:transition-transform motion-safe:duration-500': !disableAnimation }, breakpointClass.mobile, 'flex gap-4 p-2')} style={{ transform: `translateX(-${(index * 100) / currentVisibleCount}%)` }} {...useSwipeable({
        ...
        trackMouse: true,
                ...
                height: heights?.mobile,
                        ...
                        <NRCFrame priority={i === 0} component={frame.mobile?.component} image={frame.mobile?.image} onLoad={i === 0 ? toggleFirstImageLoaded : undefined} incrementCarousel={incIndex} decrementCarousel={decIndex} jumpTo={jumpTo} loadingComponent={loadingComponent} blurQuality={blurQuality} noBlur={noBlur} currentIndex={index - 1}/>
                    </div>))}
            </div>
            {!!breakpoint && (<div className={clsx({ 'motion-safe:transition-transform motion-safe:duration-500': !disableAnimation }, breakpointClass.desktop, 'flex gap-4 p-2')} style={{ transform: `translateX(-${(index * 100) / currentVisibleCount}%)` }} {...useSwipeable({
            onSwipedLeft: incIndex,
            onSwipedRight: decIndex,
            swipeDuration: 500,
            preventScrollOnSwipe: true,
            trackMouse: true,
                    ...
                    height: heights?.desktop,
                            ...
                            <NRCFrame priority={i === 0} component={frame.desktop?.component} image={frame.desktop?.image} onLoad={i === 0 ? toggleFirstImageLoaded : undefined} incrementCarousel={incIndex} decrementCarousel={decIndex} jumpTo={jumpTo} loadingComponent={loadingComponent} blurQuality={blurQuality} noBlur={noBlur} currentIndex={index - 1}/>
                        </div>))}
                </div>)}
            {!!controlsComponent &&
            controlsComponent({
