                ...
                <SheetContent className="w-screen">
                    ...
                    <div className="mt-10 flex w-full flex-col items-start py-2">
                        <div className="flex w-full items-center justify-evenly space-x-2">
                            {isSignedIn ? (<>
                                    <Link href="/profile" className={cn(buttonVariants({ variant: 'outline', size: 'sm' }), 'w-full max-w-[120px]')}>
                                        Profile
                                    </Link>
                                    <Popover>
                                        <PopoverTrigger asChild>
                                            <Button variant="outline" size="icon" className={cn(buttonVariants({ variant: 'outline', size: 'sm' }), 'relative w-full max-w-[120px]')}>
                                        ...
                                        <PopoverContent className="w-80 p-4">
                                            <div className="space-y-4">
                                                ...
                                                <Link href="/dashboard/notifications" className={cn(buttonVariants({ variant: 'outline', size: 'sm' }), 'w-full')}>
                                                    View All Notifications
                                                </Link>
                                            </div>
                                        </PopoverContent>
                                    </Popover>
                                    <Button variant="outline" className={cn(buttonVariants({ variant: 'outline', size: 'sm' }), 'w-full max-w-[120px]')} onClick={() => {
                signOut({ redirectUrl: '/' });
                handleClose();
            }}>
                                        Logout
                ...
                className: 'w-full',
                        ...
                        <ul className="mt-6 flex w-full flex-col items-start">
                            <Accordion type="single" collapsible className="!w-full">
                                {NAV_LINKS.map((link) => (<AccordionItem key={link.title} value={link.title} className="last:border-none">
                                        {link.menu ? (<>
                                            ...
                                            </>) : (<Link href={link.href} onClick={handleClose} className="flex w-full items-center py-4 font-medium text-muted-foreground hover:text-foreground" legacyBehavior>
...
const ListItem = React.forwardRef(({ className, title, href, icon: Icon, children, ...props }, ref) => {
    return (<li>
            <Link href={href} ref={ref} className={cn('block select-none space-y-1 rounded-lg p-3 leading-none no-underline outline-none transition-colors hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground', className)} {...props} legacyBehavior>
