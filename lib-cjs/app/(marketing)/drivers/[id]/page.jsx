...
const useDriverDetails = (driverId) => {
    ...
    const { buses } = (0, store_1.useBusStore)();
    ...
    const driverDetails = driver && bus
        ? (0, store_1.mapDriverAndBusToMarkerData)(driver, bus)
        : {
            id: 0,
...
const useProgress = (capacity, reservationCount) => {
...
const useNotifications = (driverTitle) => {
    const notify = (0, react_1.useCallback)(async (status, destination) => {
        ...
        const notificationBody = status === 'arrived' ? `Driver has ${status} at ${destination}.` : `Driver is now ${status}.`;
        new Notification('Driver Status Update', {
            body: notificationBody,
            icon: '/icons/notification.png',
        });
        const formData = new FormData();
        formData.append('message', status === 'arrived' && destination
        ...
        try {
            const notifyFn = {
                offline: notify_driver_status_1.notifyDriverOffline,
                'in-transit': notify_driver_status_1.notifyDriverInTransit,
                arrived: notify_driver_arrival_1.notifyDriverArrival,
            }[status];
            if (notifyFn) {
...
const CustomButton = ({ title, onPress, disabled, isActive }) => (<button className={`flex-1 py-3 text-white rounded-lg transition-colors ${disabled ? 'bg-gray-400 cursor-not-allowed' : isActive ? 'bg-green-600' : 'bg-blue-600 hover:bg-blue-700'}`} onClick={onPress} disabled={disabled} aria-pressed={isActive}>
        {title}
    </button>);
const DriverProfile = ({ driverDetails }) => (<div className="flex flex-col items-center justify-center mt-10">
...
const RideDetails = ({ capacity }) => (<div className="flex flex-col w-full items-start justify-center py-3 px-5 rounded-3xl bg-gray-100 mt-5">
...
const AddressSection = ({ userAddress, destinationAddress, }) => (<div className="flex flex-col w-full items-start justify-center mt-5">
...
const BusOccupancy = ({ progress, reservationCount, capacity, }) => (<div className="mt-5">
...
const DriverDetails = ({ driverId }) => {
    ...
    const { setActiveStatus, activeStatus } = (0, store_1.useDriverStore)();
    const { reservationCount } = (0, store_1.useReservationStore)();
    ...
    const handleStatusChange = async (status, route) => {
        ...
        try {
            await (0, names_1.fetchAPI)('/api/status-update', {
                method: 'POST',
    ...
    return (<RideLayout title="Driver Details">
            <div className="space-y-6">
                ...
                <BusOccupancy progress={progress} reservationCount={reservationCount} capacity={driverDetails.capacity}/>
