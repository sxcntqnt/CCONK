...
const CustomButton = ({ title, onPress, disabled }) => (<button className={`w-full py-3 text-white rounded-lg transition-colors ${disabled ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`} onClick={onPress} disabled={disabled}>
        {title}
    </button>);
const ProfilePage = () => {
    const { user: clerkUser } = (0, nextjs_1.useUser)();
    const { currentUser, setCurrentUser } = (0, store_1.useUserStore)();
    const { addBus } = (0, store_1.useBusStore)();
    ...
    return (<RideLayout title="My Profile">
            ...
            '/images/placeholder.png'} alt="User profile" width={110} height={110} className="rounded-full h-[110px] w-[110px] border-4 border-white shadow-sm shadow-gray-300 object-cover"/>
                ...
                <div className="bg-white rounded-lg p-5">
                    ...
                    <InputField label="Name" placeholder="Enter full name" value={profileFormData.name} onChange={handleProfileInputChange('name')} containerStyle="w-full" inputStyle="p-3.5"/>
                    ...
                    <InputField label="Phone" placeholder="Enter phone number" value={profileFormData.phoneNumber} onChange={handleProfileInputChange('phoneNumber')} containerStyle="w-full" inputStyle="p-3.5"/>
                </div>
                <div className="bg-white rounded-lg p-5">
