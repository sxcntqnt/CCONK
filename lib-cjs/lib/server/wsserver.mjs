...
async function scheduleMessageCleanup(tripId) {
...
io.on('connection', (socket) => {
    ...
    socket.on('subscribe_trip', async (data) => {
        try {
            ...
            // Send initial status
            const lastStatus = tripSubscriptions.get(tripId)?.lastStatus || trip.status || index_1.TripStatus.SCHEDULED;
    ...
    socket.on('subscribe_reservations', async (data) => {
        try {
            const { driverId } = data;
    ...
    socket.on('unsubscribe_reservations', (data) => {
        try {
            const { driverId } = data;
            if (!driverId || isNaN(Number(driverId))) {
                socket.emit('error', { error: 'Invalid or missing driverId' });
    ...
    socket.on('subscribe_chat', async (data) => {
        try {
            ...
            if (reservation.trip.status === index_1.TripStatus.COMPLETED && reservation.trip.updatedAt) {
    ...
    socket.on('send_chat_message', async (data) => {
        try {
            ...
            console.log(`Broadcasted chat message for reservation ${reservationId} from ${senderId} to ${receiverId}`);
        }
        catch (error) {
    ...
    socket.on('status_update', async (data) => {
        try {
            ...
            const tripStatusMap = {
                'in-transit': index_1.TripStatus.IN_PROGRESS,
                arrived: index_1.TripStatus.COMPLETED,
                scheduled: index_1.TripStatus.SCHEDULED,
                in_progress: index_1.TripStatus.IN_PROGRESS,
                completed: index_1.TripStatus.COMPLETED,
                cancelled: index_1.TripStatus.CANCELLED,
            };
            const driverStatusMap = {
                offline: index_1.DriverStatus.OFFLINE,
            };
            ...
            let tripId = null;
            if (driverStatus === index_1.DriverStatus.OFFLINE) {
                // Update driver status to offline
                await db.driver.update({
                    where: { id: Number(driverId) },
                    data: { status: index_1.DriverStatus.OFFLINE },
                });
                ...
                // Update trip status
                const updateResponse = await (0, index_1.updateTripStatus)(Number(tripId), tripStatus);
                if (updateResponse.error) {
                    socket.emit('error', { error: updateResponse.error });
                    return;
                }
                // Schedule message cleanup if trip is completed
                if (tripStatus === index_1.TripStatus.COMPLETED) {
            ...
            if (tripId && tripStatus && tripSubscriptions.has(tripId)) {
                ...
                await svix.message.create(subscription.appId, {
                    ...
                    payload: {
                        ...
                        destination: tripStatus === index_1.TripStatus.COMPLETED ? destination : undefined,
                        timestamp: Date.now(),
                    },
                });
...
const setupTripWebhook = async (req, res) => {
    try {
        ...
        tripSubscriptions.set(tripId, {
            ...
            lastStatus: trip.status || index_1.TripStatus.SCHEDULED,
        });
        // Send initial status
        await svix.message.create(app.id, {
            ...
            payload: {
                tripId,
                status: trip.status || index_1.TripStatus.SCHEDULED,
                timestamp: Date.now(),
            },
        });
        res.status(200).json({
...
const tripUpdatesWebhook = async (req, res) => {
    try {
        ...
        if (event.eventType === 'trip_update') {
            ...
            // Schedule message cleanup if trip is completed
            if (status === index_1.TripStatus.COMPLETED) {
                await scheduleMessageCleanup(Number(tripId));
            }
