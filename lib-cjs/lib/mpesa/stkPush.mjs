...
const mpesa_urls_1 = require("@/utils/constants/mpesa-urls");
async function initiateStkPush({ mpesa_number: phoneNumber, name, amount, }) {
    ...
    try {
        ...
        const response = await axios_1.default.post(`${mpesa_urls_1.MPESA_BASE_URL}/mpesa/stkpush/v1/processrequest`, paymentData, {
            headers: {
        ...
        const errorMessage = axiosError.response?.data?.errorMessage || axiosError.message || 'Failed to process payment request';
