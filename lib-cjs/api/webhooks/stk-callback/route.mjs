...
async function POST(request) {
    ...
    if (!data.Body?.stkCallback?.CallbackMetadata) {
        ...
        try {
            await prisma_1.db.payment.update({
        ...
        return server_1.NextResponse.json('ok', { status: 200 });
    }
    ...
    try {
        ...
        const payment = await prisma_1.db.payment.upsert({
            where: {
        ...
        return server_1.NextResponse.json({ error: 'Error saving payment' }, { status: 500 });
    }
    finally {
